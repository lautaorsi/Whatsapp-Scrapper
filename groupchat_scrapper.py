...
file = open("_chat3.txt", "r", encoding="utf8")
...
for i in lines:
    ...
    if(server_sided):
        ...
        if(hasname):
            username = (hasname.group(1)).encode('ascii','ignore')  
...
message_count = dict(sorted(message_count.items(), key=lambda item: item[1], reverse= True))
message_count = dict(itertools.islice(message_count.items(), member_count))
prevalues = list(message_count.values())
prekeys = list(message_count.keys())
postvalues = []
postkeys = []
for key in message_count:
    postvalues.append(message_count[key])
    postkeys.append(str(key).replace("b'", "").replace("'", "").replace("~", ""))
print(postkeys)
print(postvalues)
...
plt.pie(postvalues, 
        ...
        shadow=True, 
        autopct=autopct_format(postvalues), 
        wedgeprops={"edgecolor": "black", 'linewidth': 1, 'antialiased': True}, 
        radius=2.0, 
        textprops={'size': 'x-large', 'color': 'black'})

# Add a black title at the top
plt.title(f'Mensajes desde {start_date} hasta {end_date}', fontsize=16, color='black', fontweight='bold', pad=100, loc='center')



# Increase padding and adjust save settings
plt.savefig('pie.png', bbox_inches='tight', pad_inches=0.15)
